...
function jd_twit( $post_ID )  {
	...
	$oldpost = get_option( 'oldpost-edited-update' );
	$newpost = get_option( 'newpost-published-update' );
...
//add_post_meta($post_ID, 'post_cligs_text',"http://cli.gs/api/v1/cligs/create?url=".$thispostlink."&title=".$thisposttitle."&key=".$cligsapi."&appid=WP-to-Twitter"); */			
				...
				$sentence .= " - $post_status / Old: $oldpost / New: $newpost";
				if ( $sentence != '' ) {
...
/*
function jd_twit_edit( $post_ID )  {
	$jd_tweet_this = get_post_meta( $post_ID, 'jd_tweet_this', TRUE );
	$get_post_info = get_post( $post_ID );
	$post_status = $get_post_info->post_status;
	
	if ( $jd_tweet_this != "no" && $post_status == "publish") {
	    $twitterURI = "/statuses/update.xml";
	    $thispostlink = urlencode( get_permalink( $post_ID ) );
		$get_post_info = get_post( $post_ID );
		$thisposttitle = urlencode( $get_post_info->post_title );	
...
function jd_add_twitter_textinput() {
	...
	<?php _e(' characters. Twitter posts are a maximum of 140 characters; if your Cli.gs URL is appended to the end of your document, you have 119 characters available.', 'wp-to-twitter') ?> <a target="__blank" href="http://www.joedolson.com/articles/wp-to-twitter/"><?php _e('Get Support', 'wp-to-twitter') ?></a> &raquo;
</p>
<p>
...
function post_jd_twitter( $id ) {
	$jd_twitter = stripcslashes( $_POST[ 'jd_twitter' ] );
...
// Updates post meta data for all post edits.
add_action( 'edit_post','post_jd_twitter' );


if ( get_option( 'newpost-published-update' ) == '1' ) {
add_action( 'future_to_publish', 'test_function' );
...
add_post_meta( $post_ID, 'test_meta',"test" );
}
