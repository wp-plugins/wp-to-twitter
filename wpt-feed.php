...
class WPT_TwitterFeed {
  ...
  function getTweets( $count = 20, $screenname = false,$options = false ) {
    ...
    if ( $screenname == false ) $screenname = get_option( 'wtt_twitter_username' );
  ...
  private function oauthGetTweets( $screenname, $options ) {
    ...
    if ( empty( $screenname ) ) return array( 'error'=>__( 'Missing Twitter Feed Screen Name - Check Settings', 'wp-to-twitter' ) );
