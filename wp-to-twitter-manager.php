...
$wp_to_twitter_directory = get_bloginfo( 'wpurl' ) . '/' . PLUGINDIR . '/' . dirname( plugin_basename(__FILE__) );
	...
	if( get_option( 'twitterInitialised') != '1' ) {
		...
		update_option( 'oldpost-edited-update', '0' );
		...
		update_option( 'newlink-published-text', 'New link: ' );
	...
	<div>
		<fieldset>
			...
			<p>
				...
				<label for="newpost-published-update"><strong>Update Twitter when a new post is published</strong></label>
			</p>
			<p>
			...
			<p>
				...
				<label for="oldpost-edited-update"><strong>Update Twitter when an old post changes; this includes editing the post OR any time a comment is added.</strong></label>
			</p>
			<p>
